...
const tasks = {
    async createNetwork(organizations?: string, users?: string, channels?: string,
        ...
        return await CLI.createNetwork(organizations, users, channels, path, inside, pureGoResolver);
    },
...
program
    ...
    .option('-g, --use-go-resolver', 'Use pure Go resolver')
    ...
    .action(async (cmd: any) => {
        if (cmd) {
            await tasks.createNetwork(
                ...
                cmd.path, !!cmd.inside, !!cmd.useGoResolver
            );
        } else {
            await tasks.createNetwork();
        }
